    ...
    if prompt.lower().strip() == "done" and current_stage != "EXAMINER_REVIEW":
        ...
        with st.spinner("🎓 Examiner is reviewing your performance…"):
            ...
            examiner_full_instruction = (
                ...
                + "Be constructively strict — like a real Sri Lankan professor. "
                + "Then ask them to declare their Differential Diagnoses and state the expected physical signs."
            )
            examiner_contents = [
                types.Content(
                    role="user",
    ...
    # ── Examiner Viva (interactive) ───────────────────────────────────────────
        ...
        with st.spinner("🎓 Examiner is responding…"):
            viva_instruction = (
                ...
                + "Evaluate their reasoning constructively and probe further as needed. "
                + "Keep your tone strict but academic."
            )
            formatted = [
                types.Content(
                    role="user" if m["role"] == "user" else "model",
